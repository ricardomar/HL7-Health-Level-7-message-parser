        ...
        if(size(find(posicao==1),2)~=0)
            if MSA_1(campos(1,find(posicao==1)))==1
                if(size(find(posicao==2),2)~=0)
                    ...
                    mensagemStatus=strvcat(mensagemStatus,'MSA campo 2: Campo Obrigatório');
                    erroTipo1=1;
                end
            else
                mensagemStatus=strvcat(mensagemStatus,'MSA campo 1: O valor do campo não é válido');
                erroTipo1=1;
            end        
        else
            mensagemStatus=strvcat(mensagemStatus,'MSA campo 1: Campo Obrigatório');
            erroTipo1=1;
        end
    else
        mensagemStatus=strvcat(mensagemStatus,'MSA ID não reconhecido');
        erroTipo1=1;
    end
else
...
if erroTipo1==0
    ...
    if(size(find(posicao==5),2)~=0)
        if MSA_5(campos(1,find(posicao==5)))==0
            mensagemStatus=strvcat(mensagemStatus,'MSA campo 5: O valor do campo não é válido');
            erroTipo2=1;
        end
    end
